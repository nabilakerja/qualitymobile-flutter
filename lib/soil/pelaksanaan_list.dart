...
import 'package:hki_quality/soil/pelaksanaan_detail.dart';
import 'package:hki_quality/widget/appbar_theme.dart';
import 'package:hki_quality/widget/bubblebutton.dart';
import 'package:http/http.dart' as http;

class ListPelaksanaan extends StatefulWidget {
  @override
  _ListPelaksanaanState createState() =>
      _ListPelaksanaanState();
}

class _ListPelaksanaanState extends State<ListPelaksanaan> {
  List<dynamic> items = [];
  List<Map<String, dynamic>> implementations = [];
  ...
  final response = await http.get(
    Uri.parse('${DjangoConstants.backendBaseUrl}/equality/soil-implementations/'),
  );
  ...
  if (response.statusCode == 200) {
    ...
    if (decodedData.isNotEmpty) {
      for (final Map<String, dynamic> item in decodedData) {
        final int soilimplementationId = item['id'];

        final detailResponse = await http.get(
          Uri.parse('${DjangoConstants.backendBaseUrl}/equality/soil-implementations/$soilimplementationId/combined-detail/'),
        );
        ...
        if (detailResponse.statusCode == 200) {
          ...
          // Fetch name for Project
            final projectId = detailedData['implementations']['project'];
            ...
            // Fetch name for user
            final userId = detailedData['implementations']['user_id'];
          ...
          throw Exception('Failed to load detailed data for ID: $soilimplementationId');
        }
      }
    } else {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: CustomAppBar(
        title: 'List Pelaksanaan Pekerjaan Tanah',
        ),
        body: Container(
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          Text(
                                            'STA ${items[index]['implementations']['sta_start']}',
                                          ...
                                          Text(' + '),
                                          Text(
                                            '${items[index]['implementations']['sta_to']}',
                                            style: const TextStyle(
                                              fontSize: 16,
                                              fontWeight: FontWeight.bold
                                            ),
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  Text('${items[index]['implementations']['time_at']}',
                                ...
                                child: ElevatedButton(
                                  onPressed: () {
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                          builder: (context) =>
                                              PelaksanaanDetailPage(item: items[index])),
                                    );
                                  },
                                  style: ButtonStyle(
          ...
          floatingActionButton: BubbleButton(
              onPressed: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => PelaksanaanPage(username: loggedInUsername,)));
              },
            ),
    )
    );
