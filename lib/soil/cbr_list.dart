...
import 'package:hki_quality/soil/cbr_detail.dart';
import 'package:hki_quality/widget/appbar_theme.dart';
import 'package:hki_quality/widget/bubblebutton.dart';
import 'package:http/http.dart' as http;

class ListCBRSoil extends StatefulWidget {
  @override
  _ListCBRSoilState createState() =>
      _ListCBRSoilState();
}

class _ListCBRSoilState extends State<ListCBRSoil> {
  List<dynamic> items = [];
  ...
  final response = await http.get(
    Uri.parse('${DjangoConstants.backendBaseUrl}/equality/cbr-lapangan/'),
  );
  ...
  if (response.statusCode == 200) {
    ...
    if (decodedData.isNotEmpty) {
      for (final Map<String, dynamic> item in decodedData) {
        final int cbrlapanganId = item['id'];

        final detailResponse = await http.get(
          Uri.parse('${DjangoConstants.backendBaseUrl}/equality/cbr-lapangan/$cbrlapanganId/combined-detail/'),
        );
        ...
        if (detailResponse.statusCode == 200) {
          ...
          // Fetch name for Project
            final projectId = detailedData['testings']['project'];
          ...
          throw Exception('Failed to load detailed data for ID: $cbrlapanganId');
        }
      }
    } else {
                                ...
                                child: ElevatedButton(
                                  onPressed: () {
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                          builder: (context) =>
                                              CBRDetailPage(item: items[index])),
                                    );
                                  },
                                  style: ButtonStyle(
