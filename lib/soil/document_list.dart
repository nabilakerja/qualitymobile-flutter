...
class _ListDocumentState extends State<ListDocument> {
  ...
  final response = await http.get(
    Uri.parse('${DjangoConstants.backendBaseUrl}/equality/kelengkapan-dokumen-list/'),
  );
    ...
    if (decodedData.isNotEmpty) {
      for (final Map<String, dynamic> item in decodedData) {
        ...
        if (detailResponse.statusCode == 200) {
          ...
          throw Exception('Failed to load detailed data for ID: $dokumenId');
        }
      }
    } else {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: CustomAppBar(
        title: 'Kelengkapan Dokumen Persiapan Pekerjaan Tanah',
        ),
        body: Container(
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  Text('${items[index]['preparations']['time_at']}',
                                  style: const TextStyle(color: Color(0xFF8696BB)),),
                                ],
                              ),
                              Row(
